...
def parse_table(table: str) -> List[List[int]]:
    rows = table.split("\n")
    return [[int(val) for val in row.split()]
            for row in rows]


def row_checksum(table: str) -> int:
    rows = parse_table(table)
    return sum(max(row) - min(row) for row in rows)


def even_row_checksum(table: str) -> int:
    rows = parse_table(table)
...
assert even_row_checksum("""5 9 2 8
9 4 7 3
3 8 6 5""") == 9


TABLE = """626    2424    2593    139    2136    163    1689    367    2235    125    2365    924    135    2583    1425    2502
